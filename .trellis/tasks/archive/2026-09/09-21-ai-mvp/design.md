...
Restore fingerprints omit local IDs. Check duplicates first and compare source ID conflicts against the pre-restore snapshot; track repeated source IDs within a batch. New records receive Dexie-generated local IDs to prevent untrusted extreme IDs from exhausting future keys. Cancellation listeners remain attached through transaction settlement. Unchanged tag input retains its original array.
